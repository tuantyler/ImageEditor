...
def drawBlurCrop(event,canvas):
    if canvas.data.blurCropEnable==True:
        ...
        print(str(canvas.data.startCropX) + " " + str(canvas.data.startCropY) + " " + str(canvas.data.tempCropX) + " " + str(canvas.data.tempCropY))
...
def performBlur(event,canvas,blurness):
    ...
    check_deque(canvas)
    pix = canvas.data.image.load()   
    for y in range(int(round((canvas.data.startCropY-canvas.data.imageTopY)*canvas.data.imageScale)) , int(round((canvas.data.endCropY-canvas.data.imageTopY)*canvas.data.imageScale))):
        for x in range(int(round((canvas.data.startCropX-canvas.data.imageTopX)*canvas.data.imageScale)) , int(round((canvas.data.endCropX-canvas.data.imageTopX)*canvas.data.imageScale))):
            square_cut = []
...
def closeHistWindow(canvas , state=1):
    if state == 0:
...
def histogram(canvas):
    ...
    histWindow.attributes('-toolwindow', True)
    histWindow.protocol('WM_DELETE_WINDOW', lambda: closeHistWindow(canvas , 0))
    ...
    initialRGB=(0,0,0)
    changeColours(canvas, redSlider, blueSlider, greenSlider, histWindow, initialRGB)


def changeColours(canvas, redSlider, blueSlider, greenSlider, histWindow, previousRGB):
    if canvas.data.histWindowClose==True:
        ...
        if canvas.data.image!=None and histWindow.winfo_exists() :
            ...
            if currentRGB != previousRGB:
                ...
                canvas.data.imageForTk=makeImageForTk(canvas)
                drawImage(canvas)
            canvas.after(200, lambda: changeColours(canvas, redSlider,blueSlider, greenSlider,  histWindow, currentRGB))
...
def changeContrast(canvas, contrastWindow, contrastSlider, previousVal):
    if canvas.data.contrastWindowClose==True:
        ...
        if (previousVal != sliderVal):
            canvas.data.image = canvas.data.recentBeforeChange.point(lambda i: int(round(128 + (259 * (sliderVal + 255)) / (255 * (259 - sliderVal)) * (i - 128)) ))
            canvas.data.imageForTk=makeImageForTk(canvas)
            drawImage(canvas)
...
def open_image(canvas):
    ...
    img_location = fd.askopenfilename(title="Select file", filetypes=(("Image files", "*.jpg *.png"), ("all files", "*.*")))
...
def init_canvas(root):
    canvasWidth = 1280
    canvasHeight = 720
    canvas = Canvas(root, width=canvasWidth, height=canvasHeight, background="#282828" , highlightthickness=0, relief='ridge')
    class Struct: pass
    canvas.data = Struct()
...
def create_button(_frame , _btn_name , _btn_command , _side="left" , _width=12):
    init_button = Button(_frame, text=_btn_name, fg="white", background="black" , width=_width, height=1 , command=_btn_command , font=Font_tuple)
...
def extra_filter(canvas):
    ...
    create_button(blurWindow , "GaussianBlur" , lambda:filter_with_lib(canvas , ImageFilter.GaussianBlur(radius = 5)) , "top" , 20)
    create_button(blurWindow , "MedianFilter" , lambda:filter_with_lib(canvas , ImageFilter.MedianFilter(size = 3)) , "top" , 20)
    ...
    create_button(blurWindow , "EDGE_ENHANCE_MORE" , lambda:filter_with_lib(canvas , ImageFilter.EDGE_ENHANCE_MORE) , "top" , 20)
...
def init_buttons(root, canvas):
    ...
    histogram_button = create_button(toolKitFrame , "Histogram" , lambda:histogram(canvas))
